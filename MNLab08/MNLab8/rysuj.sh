...
do for [i=0:2]{
    set output "fun1_n".n[i+1].".png" 
    plot [-5:5][] f1(x) w l ls 1 t "f_1(x) = ^1/_{(1+x^2)}",\
      "file1.dat" i i u 1:3 w l ls 2 t "s(x), n = ".n[i+1].""
}
...
set key right top horizontal outside
do for [i=0:2]{
    set output "fun2_n".n[i+1].".png" 
    plot [-5:5][] f2(x) w l ls 1 t "f_2(x) = cos(2x)",\
      "file2.dat" i i u 1:3 w l ls 2 t "s(x), n = ".n[i+1].""
}
...
set output "pochodne.png"
set ylabel "f_1''(x)"
set key right bottom vertical inside
plot [-5:5] fpp(x) w l ls 1 t "f_1''(x) analityczne",\
  "pochodne.dat" u 1:2 pt 7 ps p_size*1.5 lc rgb "blue" t "m_i",\
  "" u 1:3 pt 7 ps p_size*1.5 lc rgb "red" t "f_1''(x_i) numeryczne" 
